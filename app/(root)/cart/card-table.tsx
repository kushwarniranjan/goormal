...
const CardTable = ({ cart }: { cart?: Cart }) => {
    ...
    return (<>
        ...
        {!cart || cart.items.length === 0 ? (
            <div>Cart is empty. <Link href="/" className="text-red-400">Go Shopping</Link></div>
        ) : (
                            ...
                            <TableRow key={item.slug}>
                                <TableCell>
                                    <Link href={`/product/${item.slug}`} className="flex items-center" >
                                        <Image src={item.image} alt={item.name} height={50} width={50} /> <span className="px-2">{item.name}</span>
                                    </Link>
                                </TableCell>
